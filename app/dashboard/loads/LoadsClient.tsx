...
export default function LoadsClient({ initialLoads }: LoadsClientProps) {
  ...
  const filteredLoads = useMemo(() => {
    if (!initialLoads) return []
    
    return initialLoads.filter((load) => {
      // Search filter
      ...
      // Equipment filter
      const matchesEquipment = equipmentFilter === "all" || load.equipment_type === equipmentFilter

      return matchesSearch && matchesStatus && matchesEquipment
    })
  }, [initialLoads, searchTerm, statusFilter, equipmentFilter])

  const displayedLoads = filteredLoads.slice(0, 10)
